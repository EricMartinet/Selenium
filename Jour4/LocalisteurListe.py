...
# il manque l'élément qui est pointé

print("Le nombre d\'éléments récupérés est : ", len(list_suggestions))
recherche = "selenium robot framework"
trouve = False
compteur = 0
indice = 0
for suggestion in list_suggestions:
    ...
    if recherche in suggestion.text:
        print(recherche,"*** trouvé !!***")
        trouve = True
        indice = compteur
#        break           # Ne pas sortir de la boucle pour compter tous les items
...
# 7- Compléter la saisie avec "r" pour obtenir "selenium r"
pilote.find_element(By.XPATH, "//input[@class='gLFyf gsfi']").send_keys("r")
time.sleep(3)

# prendre un instantanné :
pilote.save_screenshot("listeSuggestions2.png")
# 5B- sélectionner le choix prérenseigné à selenium robotframework
list_suggestions = pilote.find_elements(By.XPATH, "//div[@class='wM6W7d']/span")
print("Le nombre d\'éléments récupérés est : ", len(list_suggestions))
recherche = "selenium robot framework"
trouve = False
...
for suggestion in list_suggestions:
    ...
    if recherche in suggestion.text:
        ...
        break
if not trouve:
    # 6B - vérifier le nombre de suggestions = 10?
    print("Le nombre de suggestions est :", compteur)
    print(recherche,"n'est pas dans la liste")
else :
    list_suggestions[indice-1].click()
